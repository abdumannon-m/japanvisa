...
- `/api/health`: non-secret deployment health endpoint. It reports whether Telegram, Upstash, Supabase, webhook, and cron secrets are configured.

For Vercel to send Telegram messages and remember subscribers, it needs a durable state backend. The recommended setup is **Upstash Redis** using `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`. If you do not have a database available, use the GitHub Actions polling mode above.
...
- The existing GitHub Actions workflow can use the same Upstash environment variables and `STATE_KEY=event-20` as a backup watcher. It will dedupe against Vercel runs and avoid double alerts.
